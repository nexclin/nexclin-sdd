-- Add "Outro" objection to existing clinics that don't already have it
INSERT INTO public.objections (clinic_id, name, active)
SELECT c.id, 'Outro', true
FROM public.clinics c
WHERE NOT EXISTS (
  SELECT 1 FROM public.objections o
  WHERE o.clinic_id = c.id AND o.name = 'Outro'
);

-- Update handle_new_user to include "Outro" at the end of seeded objections
